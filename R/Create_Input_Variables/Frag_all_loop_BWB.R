...
plot(ras)
b<-ClassStat(ras,25) # Input Raster + Cell Size
head(b)
b$NumClasses_J<-sum(as.numeric(b[,1]!=99)) #CALCULATE NUMBER OF CLASSES TAKING 128 (NODATA) AWAY
b$ED_total_J<-sum(as.numeric(b[b$class!=99,7])) #SUM ED (SUM ED OF DIFFERENT CLASSES)
b$Area_total_Km2<-sum(as.numeric(b[b$class!=99,3]))/1000000#Km2
b$Mean_Patch_areaKm2<-sum(as.numeric(b[b$class!=99,10]))/1000000#km2
filename<-as.character(names(ras))
str(b)
b$landscape<-filename  	#CAPTURE THE LANDSCAPE NUMBER I THE ROW
b<-b[,c(39:43)]	#EXTRACT ONLY IMPORTANT COLUMNS
head(b)
write.table(b,paste("data/KM_rasters/", filename,".txt", sep=""))
